...
module.exports = {
    ...
    description: 'Set your collection privacy (private/unprivate)',
    async execute(message) {
        ...
        const sentMsg = await message.channel.send({
            content: 'Choose your collection privacy setting:',
            components: [row]
        });
        ...
        collector.on('collect', async interaction => {
            ...
            await pool.execute(
                'INSERT INTO user_inventory (user_id, private_collection) VALUES (?, ?) ON DUPLICATE KEY UPDATE private_collection = ?',
                [message.author.id, value, value]
            );
            await interaction.update({
                content: `Your collection is now ${mode}.`,
                components: []
            });
        });
